...
-- ACL table size
acl_ts = 5
acl_tbl = Dynamic("acl0",(acl_ts,["srcmac","dstmac"]))
acl_val = Static([[("srcmac",0),("dstmac",0)] | x <- [1..acl_ts]])
acl_ptrn = [("srcmac","dstmac")]
...
-}
    , BRTF(acl_tbl,"i","lbl_drop")             -- if match on acl_tbl then set index field "j" with the matched index and jump to "lbl_drop" else next
    , JMP("lbl_end")
    , LBL("lbl_drop")                          -- label "lbl_drop"
    , DRP
